...
private:
    ...
    template<typename T>
    u64 skipRelArrayViewHeader(hkRelArrayView<T, int>*) {
        u64 at = getOffset();
    ...
    void writeRelArrayViewHeader(hkRelArrayView<T, int>* obj, u64 dataOff, u64 headerOff = 0) {
        ...
        writeInteger((uint32_t)(dataOff - getOffset()), sizeof(uint32_t));
        writeInteger(obj->m_size, sizeof(uint32_t));
        seek(dataOff, SeekType::Begin);
    }
    template<typename T>
    u64 skipRelArrayHeader(hkRelArray<T>*) {
        u64 at = getOffset();
    ...
    template<typename T>
    u64 skipRelPtrHeader(hkRelPtr<T>*) {
        u64 at = getOffset();
        seek(8, SeekType::Current);
        return at;
    }
    ...
    enum class SeekType { Begin, Current };
    static const u64 cReserveChunkSize = 1024 * 1024;
    ...
    u64 mOffset = 0;
    u64 mShapeTagTableOffset = 0;
    u64 mTopLevelTreeOffset = 0;
    u64 mGeometrySectionsOffset = 0;
